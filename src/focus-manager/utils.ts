...
const filterDirectionOnlyFnMap: ArrowKeyMap<
  (source: NodeRect) => (target: NodeRect) => boolean
> = {
...
const filterDirectionOnlyTargets = (
  source: NodeRect,
  targets: NodeRect[],
  arrowKey: ArrowKey,
) => {
...
const getDistanceMap: ArrowKeyMap<
  (source: NodeRect, target: NodeRect) => number
> = {
...
const getVerticalIntersectionAmount = (
  source: NodeRect,
  target: NodeRect,
): number => {
  return (
    Math.min(source.rect.right, target.rect.right) -
    Math.max(source.rect.left, target.rect.left)
  );
};

const getHorizontalIntersectionAmount = (
  source: NodeRect,
  target: NodeRect,
): number => {
  return (
    Math.min(source.rect.bottom, target.rect.bottom) -
    Math.max(source.rect.top, target.rect.top)
  );
};

const getIntersectionMap: ArrowKeyMap<
  (source: NodeRect, target: NodeRect) => number
> = {
...
const filterMostIntersectedTargets = (
  source: NodeRect,
  targets: NodeRect[],
  arrowKey: ArrowKey,
) => {
  ...
  const max = intersections.reduce(
    (agg, { value }) => Math.max(agg, value),
...
export const findNext = (source: Node, targets: Node[], arrowKey: ArrowKey) => {
  ...
  const directionOnlyTargets = filterDirectionOnlyTargets(
    sourceRect,
    targetRects,
    arrowKey,
  );
  const nearestTargets = filterNearestTargets(
    sourceRect,
    directionOnlyTargets,
    arrowKey,
  );
  const mostIntersectedTargets = filterMostIntersectedTargets(
    sourceRect,
    nearestTargets,
    arrowKey,
  );
